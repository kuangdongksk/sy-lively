...
- **提示词模板**: 支持自定义提示词模板，快速填充常用提示词
- **思考模式**: 支持DeepSeek思考模式，显示AI推理过程
- **实时预览**: 流式响应时实时显示生成内容
- **上下文检查**: 自动检查上下文长度，超出限制时发出警告
...
| 启用 | 是否启用此提供商 | 勾选 |
| 流式响应 | 是否使用流式响应 | 勾选 |
| 思考模式 | 是否启用DeepSeek思考模式 | 勾选 |
| 最大Token数 | 设置最大生成token数 | `4096` |
...
提示词模板功能可以让你快速使用常用的提示词，减少重复输入。

### 使用模板

1. 在AI聊天对话框中，点击"提示词模板"按钮
2. 选择快捷模板按钮直接应用
3. 或点击"查看全部"打开模板管理对话框
4. 点击模板即可将内容填充到输入框

### 管理模板

在模板管理对话框中可以：
- **新建模板**: 点击"新建模板"按钮，输入名称和内容
- **编辑模板**: 点击模板项的编辑按钮修改内容
- **删除模板**: 点击模板项的删除按钮
- **设为快捷**: 勾选"快捷方式"将在聊天界面显示为快捷按钮
...
AI聊天会自动保存对话历史，支持连续对话。

- 每个提供商独立保存最多20条对话记录
- 对话历史包含用户问题、AI回答和思考过程
- 新对话会自动包含历史记录作为上下文
- 对话历史保存在本地，不会上传到服务器
...
**技术改进**
- 🔧 新增 `PromptTemplateService` 管理提示词模板
- 🔧 优化流式响应处理，支持思考过程和内容分离
- 🔧 改进上下文检查机制，基于字符数估算
- 🔧 完善对话历史管理，按提供商独立存储
- 🔧 设置界面新增思考模式和最大Token数选项
